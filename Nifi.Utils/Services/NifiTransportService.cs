...
namespace Nifi.Utils.Services
{
    public class NifiTransportService : INifiTransportService
    {
        ...
        public NifiTransportService(HttpClient http_client, IFlowFileService flow_file_service)
        {
        ...
        public async Task<bool> SendViaS2SAsync(string nifi_base_url, string input_port_name, IEnumerable<NifiPackage> packages)
        {
            ...
            NifiPortDto? port = s2_s_info?.controller?.input_ports?.FirstOrDefault(p => p.name != null && p.name.Equals(input_port_name, StringComparison.OrdinalIgnoreCase));
