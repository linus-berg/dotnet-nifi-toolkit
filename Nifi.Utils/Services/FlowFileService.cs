...
public class FlowFileService : IFlowFileService {
  ...
  public async Task<byte[]> CreateFlowFileV3Async(
    IDictionary<string, string> attributes, byte[] content) {
    using MemoryStream ms = new();
    using MemoryStream content_stream = new(content);
    await WriteFlowFileV3Async(attributes, content_stream, ms);
    return ms.ToArray();
  }

  public async Task WriteFlowFileV3Async(IDictionary<string, string> attributes,
  ...
  public async Task<byte[]> CreateFlowFileV1Async(
    IDictionary<string, string> attributes, byte[] content) {
    using MemoryStream ms = new();
    using MemoryStream content_stream = new(content);
  ...
  public async Task WriteFlowFileV1Async(IDictionary<string, string> attributes,
    ...
    UstarTarEntry attr_entry =
      new(TarEntryType.RegularFile, "flowfile.attributes") {
  ...
  private async Task<string> ReadStringV3Async(Stream stream,
  ...
  private async Task<int> ReadFieldLengthAsync(Stream stream,
                                               CancellationToken ct) {
    byte[] bytes = new byte[2];
