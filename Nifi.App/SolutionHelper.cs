...
public static class SolutionHelper {
  public static string GetSolutionDirectory() {
    ...
    DirectoryInfo? current_directory = new(AppContext.BaseDirectory);

    // Walk up the directory tree until we find a .sln file
    while (current_directory != null &&
           !current_directory.GetFiles("*.sln").Any()) {
    ...
    if (current_directory == null) {
      throw new DirectoryNotFoundException(
        "Could not find the solution directory. No .sln file found in the directory tree."
      );
    }
